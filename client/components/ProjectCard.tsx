...
export const ProjectCard: React.FC<ProjectCardProps> = ({ id, title, description, category, tags, image }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header Block */}
        <div className="flex flex-col gap-0.5 mb-2.5">
          <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-white/40 block leading-normal">
            Project Dossier:
          </span>
          <h3 className="text-white font-black text-xs uppercase tracking-wide leading-snug mt-0.5">
            {title}
          </h3>
        </div>

        {/* Middle Content Container */}
        <div className="flex-1 min-h-0 flex flex-col justify-start gap-2 my-2.5">
          <div>
            <span className="font-mono text-[8px] uppercase tracking-[0.18em] text-white/40 block mb-0.5 leading-normal">
              Overview
            </span>
            <p className="text-white/75 text-[9.5px] font-sans leading-relaxed">
              {description}
            </p>
          </div>
          ...
          <ul className="space-y-1 mt-1">
            {tags.map((tag) => (
              <li key={tag} className="flex items-center gap-1.5 text-white/65 text-[9px] font-sans">
        ...
        <div className="mt-auto grid grid-cols-2 gap-2 pt-2.5 border-t border-white/10">
          <Link
            href={`/projects/${id}`}
            onClick={(e) => e.stopPropagation()}
            className="group flex items-center justify-center gap-1 py-2 px-1.5 rounded-xl bg-red-950/40 border border-red-300/20 text-white font-mono text-[7.5px] uppercase tracking-[0.05em] hover:bg-red-300/20 hover:border-red-300/40 hover:text-red-300 transition-all duration-300 text-center leading-normal"
          >
